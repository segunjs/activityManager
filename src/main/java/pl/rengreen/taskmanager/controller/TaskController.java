...
import pl.rengreen.taskmanager.serviceImplementation.MailService;
...
public class TaskController {
    ...
    @Autowired
    public TaskController(TaskService taskService, UserService userService, MailService mailService) {
    ...
    public String showEmptyTaskForm(Model model, Principal principal, SecurityContextHolderAwareRequestWrapper request, HttpSession session) {
        String email = principal.getName();
    ...
    public String createTask(@Valid Task task, BindingResult bindingResult, HttpSession session) throws MessagingException {
        if (bindingResult.hasErrors()) {
            return "forms/task-new";
        }
        User user = (User) session.getAttribute("user");
        taskService.createTask(task);
        mailService.sendEmail(task, user);
